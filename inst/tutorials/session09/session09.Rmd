...
Before we start running regressions with multiple explanatory variables, let's recap why we might want to do this in the first place. As discussed in class, one of the main reasons for multiple regression is to address the issue of omitted variable bias. It implies that we systematically miss the true parameter in the population model when we run our statistical model. 
...
Last week, we found that size of a party (`seatshare`) is associated with an increase in how often a party votes against the government. However, over the course of this week, you couldn't stop thinking about it and started fearing that there may be unobserved factors that confound the relationship both at the party level as well as at the system level. 

Specifically, one might expect that larger parties are on average be ideologically closer to government parties than smaller (fringe) parties. Moreover, it is possible that larger parties have more government experience and are, therefore, likely to behave differently vis-a-vis the government than smaller parties. 

In addition, there may be party-system specific factors varying between countries that affect how party size and opposition to the government are related in our data. For example, the degree to which institutional arrangements allow opposition parties to influence decision-making, the general openness of the competition structure of a party system, the general importance of the parliament for policy-making, and what type of government the opposition parties is facing may be theoretically plausible factors that affect the relationship between party size and opposition behavior.
...
model <- lm(conflict_p ~ abs_lrdiff + seatshare + ge + inst_power + 
              y_openness + importance + govtype_mod, data = df_opposition)
knitreg(model)
```
...
question_text(
  ...
  answer("0.69", correct = T),
  answer("69", correct = F, message = "Close, but this time you were supposed to report the actual size of the estimate without transforming it the into percentage share."),
  allow_retry = T
  )
```
...
question_text(
  ...
  answer("12", correct = T),
  answer("11.51", correct = F, message = "Close, but this is too detailed. Please provide the answer with two meaningful digits only."),
...
Usually, we should be very carefully interpreting control variables by themselves, at least if we didn't spent much time thinking about potential confounders affecting the relationship between this particular control variable and the outcome of interest. 
...
question("What do regression estimates tell us about the relationship between government types (`govtype_mod`) and the share of opposing votes to the government?",
  answer("A one unit increase in `govtype_mode` causes an increase of about 1.7 percentage points in votes against the government, holding everthing else contant.", correct = F, message = "Be very careful to make causal statements when using regression analysis without a randomization strategy. Moreover, you are dealing with a dummy variable so it is recommendable to provide interpretations for the respective category values."),
    answer("Surplus coalition governments face on average 1.7 percentage points more opposition than minimal-winning coalition governments, holding everthing else contant.", correct = T),
  answer("Opposition parties facing a surplus coalition government vote on average 1.7 percentage points more often against the government than opposition parties facing a minimal-winning coalition, holding everthing else contant.", correct = T),
  answer("Opposition parties facing a minimal-coalition government vote on average 1.7 percentage points more often against the government than opposition parties facing a surplus coalition government, holding everthing else contant.", correct = F, message = "Think about the reference category."),
  answer("Opposition parties facing a minimal-coalition government vote against the government in 78% of the votes on average, setting all other variables to zero.", correct = T),
  random_answer_order = TRUE,
  allow_retry = T
)
