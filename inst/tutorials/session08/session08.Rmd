...
After getting to know `R` and the `tidyverse` in previous weeks, it's now time to run our first statistical model. Before we can do so, we'll have to load the data and should get familiar with it. Luckily, the data set is already preloaded and called `df_opposition`. It is the same data we used in the lab on opposition parties in national European parliaments. Let's check it out. 

**How many observations and variables does the data have?**
...
As in the lab session, we want to better understand why and how often opposition parties vote against the government. Hence, the  percentage share of votes that are opposed to the government is our dependent variable. The corresponding variable in the data set is called `conflict_p`. 
...
question("The median  of the conflict measure (in percent and rounded to two meaningful digits) is:",
  answer("54", correct = T),
...
question_text(
  "The standard deviation of the conflict measure (in percent and rounded to two meaningful digits) is:",
...
Let's find out whether larger opposition parties oppose the government more often than smaller ones. As a proxy for party size we are taking its seat share stored in the variable `seatshare`. Before we run the regression analysis, we should know how the variable looks like.
...
Now that you got a first visual glimpse at the explanatory variable, let's learn about some summary statistics.
...
question_text(
  "The average value of `seatshare` (in percent and rounded to two meaningful digits) is:",
  answer("12", correct = T),
  allow_retry = T
  )
```
...
question_text(
  "The maximum seat share of an opposition party in the data (in percent and rounded to two meaningful digits) is:",
  answer("41", correct = T),
  allow_retry = T
  )
```
...
question("What does the coefficient estimate tell us about the relationship between seat share and the share of opposing votes to the government?",
  ...
  answer("An increase of one percentage point in vote share is associated with a an increase of about 0.56 percentage points in the the share of votes against the government.", correct = T),
    answer("We expected an increase of about 56 percentage points in the share of votes against the government when the seat share variable increases by one unit.", correct = T),
  random_answer_order = TRUE,
  allow_retry = T
)
...
As always, it's important to keep in mind what we are assuming the models that we run on our data to **infer** what is going on in the real world.
